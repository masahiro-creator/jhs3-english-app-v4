// 学習ダッシュボード コンポーネント
// 不規則動詞と単語は「覚え直しの間隔（エビングハウス忘却曲線）」という同じ仕組みで
// 学習する同じカテゴリの復習項目として扱い、指標もミッションも1つにまとめて表示する。
window.DashboardComponent = {
  render(containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;

    const verbs = window.IRREGULAR_VERBS || [];

    const verbStats = window.SRSEngine.getStats(verbs);
    const streak = window.StorageEngine.updateStreak();
    const verbWeakItems = window.SRSEngine.getWeakItems(verbs);
    const word = window.WordDrill ? window.WordDrill.getSummary() : null;

    // 動詞・単語をまとめた全体の指標
    const totalItems = verbStats.totalItems + (word ? word.totalCount : 0);
    const totalMastered = verbStats.masteredCount + (word ? word.masteredCount : 0);
    const retentionRate = totalItems > 0 ? Math.round((totalMastered / totalItems) * 100) : 0;
    const totalTasks = verbStats.dueCount + (word ? word.dueCount + word.freshCount : 0);

    const wordPerDayButtonsHtml = word
      ? word.newPerDayOptions.map(n => `
          <button class="option-btn" style="padding: 8px 14px; min-height: 38px; font-size: 13px; background: ${word.newPerDay === n ? 'var(--theme-btn-grad)' : 'rgba(255,255,255,0.05)'};" onclick="window.DashboardComponent.setWordNewPerDay(${n})">
            ${n}語
          </button>
        `).join('')
      : '';

    container.innerHTML = `
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-val">🔥 ${streak}日</div>
          <div class="stat-lbl">連続学習ストリーク</div>
        </div>
        <div class="stat-card">
          <div class="stat-val">🧠 ${retentionRate}%</div>
          <div class="stat-lbl">記憶定着率（動詞＋単語）</div>
        </div>
        <div class="stat-card">
          <div class="stat-val">⏰ ${totalTasks}件</div>
          <div class="stat-lbl">今日のタスク数（動詞＋単語）</div>
        </div>
        <div class="stat-card">
          <div class="stat-val">🏆 ${totalMastered}件</div>
          <div class="stat-lbl">殿堂入りマスター数（動詞＋単語）</div>
        </div>
      </div>

      <div class="glass-card" style="padding: 32px 20px;">
        <h2 style="font-size: 24px; font-weight: 800; margin-bottom: 4px; color: #ffffff; text-align: center;">
          忘却曲線に打ち勝つ！今日の学習ミッション
        </h2>
        <p style="font-size: 13px; color: var(--theme-text-sub); margin-bottom: 24px; text-align: center;">
          動詞も単語も同じ仕組み（覚え直しの間隔）で復習するよ
        </p>

        <div style="border-top: 1px solid var(--theme-card-border); padding-top: 20px;">
          <h3 style="font-size: 17px; font-weight: 800; margin-bottom: 10px; text-align: center;">🔥 不規則動詞：今日復習すべき ${verbStats.dueCount}問</h3>
          <div style="display: flex; gap: 14px; justify-content: center; flex-wrap: wrap;">
            <button class="option-btn" style="background: var(--theme-btn-grad); min-width: 220px; font-size: 18px;" onclick="window.App.startSRSSession()">
              ⚡ 動詞の復習を開始 (${verbStats.dueCount})
            </button>
            ${verbWeakItems.length > 0 ? `
              <button class="option-btn" style="background: rgba(239, 68, 68, 0.2); border-color: #ef4444; min-width: 220px; font-size: 18px;" onclick="window.App.switchTab('weak')">
                🎯 動詞の弱点克服 (${verbWeakItems.length}件)
              </button>
            ` : ''}
          </div>
        </div>

        ${word ? `
          <div style="border-top: 1px solid var(--theme-card-border); margin-top: 24px; padding-top: 20px;">
            <h3 style="font-size: 17px; font-weight: 800; margin-bottom: 10px; text-align: center;">📚 単語：今日の復習 ${word.dueCount}語 ／ 目標語数のこり ${word.freshCount}語（目標${word.newPerDay}語）</h3>
            <div style="display: flex; gap: 14px; justify-content: center; flex-wrap: wrap; margin-bottom: 18px;">
              ${word.dueCount + word.freshCount > 0 ? `
                <button class="option-btn" style="background: var(--theme-btn-grad); min-width: 220px; font-size: 18px;" onclick="window.DashboardComponent.startWordSession()">
                  🚀 単語の復習を開始
                </button>
              ` : `
                <p style="font-size: 15px; color: var(--theme-text-sub); margin: 0;">🎉 今日の単語分は終わったよ！また明日会おうね。</p>
              `}
              ${word.weakCount > 0 ? `
                <button class="option-btn" style="background: rgba(239, 68, 68, 0.2); border-color: #ef4444; min-width: 220px; font-size: 18px;" onclick="window.DashboardComponent.startWordWeak()">
                  🎯 単語の弱点克服 (${word.weakCount}件)
                </button>
              ` : ''}
            </div>

            <div style="font-size: 13px; color: var(--theme-text-sub); margin-bottom: 8px; text-align: center;">1日に出す新しい語</div>
            <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">
              ${wordPerDayButtonsHtml}
            </div>
          </div>
        ` : ''}
      </div>

      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
        <div class="glass-card" style="cursor: pointer;" onclick="window.App.switchTab('verbs')">
          <h3 style="font-size: 20px; font-weight: 800; margin-bottom: 8px;">🔥 不規則動詞デイリーマスター</h3>
          <p style="font-size: 14px; color: var(--theme-text-sub);">AAA/ABB/ABA/ABC型 全70語の活用形＆音声再生トレーニング</p>
        </div>
        <div class="glass-card" style="cursor: pointer;" onclick="window.App.switchTab('words')">
          <h3 style="font-size: 20px; font-weight: 800; margin-bottom: 8px;">📖 単語一覧・成績・記録</h3>
          <p style="font-size: 14px; color: var(--theme-text-sub);">全1394語の一覧検索・分野別成績・これまでの記録グラフ</p>
        </div>
      </div>
    `;
  },

  startWordSession() {
    if (!window.WordDrill) return;
    window.App.switchTab('words');
    window.WordDrill.startSession();
  },

  startWordWeak() {
    if (!window.WordDrill) return;
    window.App.switchTab('words');
    window.WordDrill.startWeakSession();
  },

  setWordNewPerDay(value) {
    if (!window.WordDrill) return;
    window.WordDrill.setNewPerDay(value);
    this.render('tab-dashboard');
  }
};
